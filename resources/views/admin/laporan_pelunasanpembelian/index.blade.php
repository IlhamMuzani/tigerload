...
@section('content')
                        ...
                        <tbody>
                            @foreach ($inquery as $fakturpelunasan)
                                <tr id="editMemoekspedisi" data-toggle="modal"
                                    ...
                                    <td>
                                        @if ($fakturpelunasan->pembelian)
                                            {{ $fakturpelunasan->pembelian->user->karyawan->nama_lengkap }}
                                        @else
                                            tidak ada
                                        @endif
                                    </td>
                                    <td>
                                        @if ($fakturpelunasan->faktur_pelunasanpembelian)
                                            {{ $fakturpelunasan->faktur_pelunasanpembelian->nama_supplier }}
                                        @else
                                            tidak ada
                                        @endif
                                    </td>
    ...
    <script>
        ...
        function cari() {
            form.action = "{{ url('admin/laporan_fakturpelunasanpembelian') }}";
            form.submit();
        }
        ...
        function printReport() {
            ...
            if (startDate && endDate) {
                form.action = "{{ url('admin/print_pelunasanpembelian') }}" + "?start_date=" + startDate + "&end_date=" +
                    endDate;
                form.submit();
            } else {
            ...
            $('#statusx').on('change', function() {
                ...
                switch (selectedValue) {
                    case 'memo_perjalanan':
                        window.location.href = "{{ url('admin/laporan_fakturpelunasanpembelian') }}";
                        break;
                    case 'memo_borong':
                        window.location.href = "{{ url('admin/laporan_pelunasanglobalpembelian') }}";
                        break;
                    default:
                        break;
                }
