...
<head>
    ...
    <title>Laporan Return Pembelian</title>
    <style>
        body {
            margin: 0;
            padding: 20px;
...
<body>
    <div class="container">
        <h1>LAPORAN RETURN PEMBELIAN - RANGKUMAN</h1>
    </div>
    ...
    <table>
        ...
        @foreach ($inquery as $pembelian)
            <tr>
                <td>{{ $pembelian->kode_pembelianreturn }}</td>
                ...
                <td>{{ 'Rp ' . number_format($pembelian->detail_pembelianreturn->sum('harga'), 0, ',', '.') }}
                </td>
            </tr>
            @php
                $total += $pembelian->detail_pembelianreturn->sum('harga');
            @endphp
        @endforeach
    </table>
