...
<body>
    <div class="container">
        <h1>LAPORAN PELUNASAN - RANGKUMAN</h1>
    </div>
    ...
    <table>
        <tr>
            <th style="width: 35%;">Faktur Pelunasan</th>
        ...
        @foreach ($inquery as $pelunasan)
            <tr>
                ...
                {{-- <td>
                    @if ($pelunasan->kendaraan->merek)
                        {{ $pelunasan->kendaraan->merek->nama_merek }}
                    @else
                        data tidak ada
                    @endif
                </td> --}}
            ...
            {{-- total masih belum sempurna dijumlahkanya  --}}
            @php
                $total += $pelunasan->penjualan->depositpemesanan->spk->harga - $pelunasan->penjualan->depositpemesanan->harga + $pelunasan->penjualan->detail_penjualan->where('penjualan_id', $pelunasan->penjualan->id)->sum('harga'); // Tambahkan harga ke total
            @endphp
        @endforeach
    </table>
